...
def strip_annotation_simple(out):
    # remove position-only and keyword-only markers
    for sep in ('/', '*'):
        before, _, after = out.partition(sep)
        if after:
            before = before[:-2]  # remove tailing ', '
        out = before + after
    ...
    while True:
        ...
        if idx_start == -1:
            break
        idx_end = out[idx_start:].find(',')
        if idx_end == -1:
            idx_end = out[idx_start:].find(')')
        idx_end += idx_start
        out = out[:idx_start] + out[idx_end:]

    return out

def strip_annotation(out):
    # remove position-only and keyword-only markers
    for sep in ('/', '*'):
        before, _, after = out.partition(sep)
        if after:
    ...
    # remove output type annotation
    idx = out.rfind(')')
    out = out[:idx+1]
    
    # remove argument type annotation
    # the annotation could be a nested bracket like Optional[Union[int, Tuple[int, int]]],
    # so regex is not the best option (nor do I know it well enough to use it)
    def parse_and_remove_brackets(out):
        def _helper(idx_leftmost):
            idx_leftmost += 1
            count = 1
            i = 0
            while True:
                if count == 0:
                    idx_rightmost = idx_leftmost + i
                    break
                if out[idx_leftmost + i] == ']':
                    count -= 1
                if out[idx_leftmost + i] == '[':
                    count += 1
                i += 1
            return idx_rightmost
    
        # remove all brackets from the expression, as it could contain ',' etc
        while True:
            idx_leftmost = out.find('[')
            if idx_leftmost == -1:
                return out
            # do a linear search for the matching bracket
            idx_rightmost = _helper(idx_leftmost)
            out = out[:idx_leftmost] + out[idx_rightmost:]
        return out   
    
    out = parse_and_remove_brackets(out)
